		...
		//verificar se existe a pasta teste
				...
				try {
					...
					if(vazio) {
						...
						System.out.println("O arquivo não possui o total de linhas necessarias. criando...");
						OutputStream os = new FileOutputStream(caminhoConfigTxt);
						OutputStreamWriter osw =  new OutputStreamWriter(os);
						BufferedWriter writer = new BufferedWriter(osw);
				...
				List<File> filteredFiles = Arrays.asList(diretorioTeste.listFiles())
						.stream()
						.filter(file -> {
				...
				for (int i = 0; i < filteredFiles.size(); i++) {
					...
					try {
						...
						if ( !headerTotalNumberOfNodes.toString().equals(String.format("%02d", uniqueNodesTotal.size())) ||
								!String.format("%02d", uniqueNodesTotal.size()).equals(headerTotalNumberOfNodes.toString()) 
								) {
							throw new Exception("Número totais de nós inválido");
						}
						if (!headerSumOfArestsWeights.toString().equals(sumOfWeights.toString()) ||
							!sumOfWeights.toString().equals(headerSumOfArestsWeights.toString())||
							!sumOfWeights.toString().equals(trailerSumOfWeights.toString())||
							!trailerSumOfWeights.toString().equals(headerSumOfArestsWeights.toString())||
							!trailerSumOfWeights.toString().equals(sumOfWeights.toString())) {
							throw new Exception("Soma dos pesos difere (Valor do Registro HEADER = NN e Soma dos Pesos = NN), Ou Difere do Trailer");
						}
						if ( !headerTotalNumberOfNodes.toString().equals(String.format("%02d", trailerNumberOfConnectionLines))||
								!headerTotalNumberOfNodes.toString().equals(String.format("%02d", contagemLinhasDeNos)) ||
								!headerTotalNumberOfNodes.toString().equals(String.format("%02d", contagemLinhasPesos))) {
						...
						try {
							nomeArquivoProcessado = caminhodiretorioProcessados + "\\" + currentFile.getName();
							...
							if(criadoTxtComSucesso) {
								...
								FileReader fileReader2 = new FileReader(currentFile);
						...
						// TODO Auto-generated catch block
						try {
							nomeArquivoNaoProcessado = caminhodiretorioNaoProcessados + "\\" + currentFile.getName();
						...
						System.out.println("caiu aqui aaaaa");
						try {
							nomeArquivoNaoProcessado = caminhodiretorioNaoProcessados + "\\" + currentFile.getName();
							File GrafoProcessado = new File(nomeArquivoNaoProcessado);
							boolean criadoTxtComSucesso;
